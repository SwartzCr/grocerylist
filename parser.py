...
def renumber(grocery_list):
    return list(enumerate([grocery_item for
                           (old_index, grocery_item) in grocery_list]))
...
class AutomtedTest(unittest.TestCase):
    ...
    def test_parsed_email_to_split_lines(self):
        with open(dir_name + "/test.txt") as f:
            test_email = f.read()
        parsed_email = parse_email(test_email)
        result = parsed_email2split_lines(parsed_email)
        self.aserEqual(result, ["hsa", "ashd", "ashd"])
    ...
    def test_remove_items(self):
        result = remove_items("r 2 3", [(0, 'a'), (1, 'b'), (2, 'c'),
                                        (3, 'd'), (4, 'e'), (5, 'f')])
        self.assertEquals(result, [(0, 'a'), (1, 'b'), (4, 'e'), (5, 'f')])

    def test_remove_trailing_whitespace(self):
        result = execute([("r 2 3 ")], [(0, 'a'), (1, 'b'), (2, 'c'),
                                        (3, 'd'), (4, 'e'), (5, 'f')])
...
def main(email_data):
    ...
    if email_line_list:
        path = dir_name + "/archives/" + datetime.datetime.now().ctime()
        with open(path, 'w') as er:
            er.write(email_data)
